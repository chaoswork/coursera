...
%disp(allelesToGenotypes);
%disp(genotypesToAlleles);
...
for i=1:2
  for j=1:3
    for k=1:3
      ...
      %disp("idx="), disp(allelesToGenotypes(j, k));
      %disp("ass to index="), disp(AssignmentToIndex([i,j,k],phenotypeFactor.card));
      %disp("-------");
      prob = alphaList(allelesToGenotypes(j, k));
      if i == 2
	prob = 1 - prob;
      endif
      phenotypeFactor.val(AssignmentToIndex([i,j,k],phenotypeFactor.card)) = prob;
    endfor
  endfor
endfor
