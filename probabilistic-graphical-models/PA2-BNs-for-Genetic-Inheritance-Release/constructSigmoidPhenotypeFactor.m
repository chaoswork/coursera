...
phenotypeFactor.var = [phenotypeVar, geneCopyVarOneList(1, 1), geneCopyVarOneList(2, 1), geneCopyVarTwoList(1, 1), geneCopyVarTwoList(2,1)];
...
for i = 1:2
  for j = 1:2
    for k = 1:2
      sum_jk = alleleWeights{1}(j) + alleleWeights{2}(k);
      for p = 1:2
	for q = 1:2
	  indx = AssignmentToIndex([i,j,k,p,q],phenotypeFactor.card);
	  sum_pq = alleleWeights{1}(p) + alleleWeights{2}(q);
	  prob = computeSigmoid(sum_jk+sum_pq);
	  if i == 2
	    prob = 1 - prob;
	  endif
	  phenotypeFactor.val(indx) = prob;
	endfor
      endfor
    endfor
  endfor
