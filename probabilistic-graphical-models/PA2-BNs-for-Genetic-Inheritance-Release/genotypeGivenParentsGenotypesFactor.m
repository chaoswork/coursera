...
genotypeFactor.card = [length(genotypesToAlleles), length(genotypesToAlleles), length(genotypesToAlleles)];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

for i=1:length(genotypeFactor.val)
  ass = IndexToAssignment(i, genotypeFactor.card);
  child = sort(genotypesToAlleles(ass(1), :));
  father = genotypesToAlleles(ass(2), :);
  mother = genotypesToAlleles(ass(3), :);
  %printf("(%d,%d)|(%d,%d),(%d,%d)\n", child(1), child(2), father(1), father(2), mother(1), mother(2));
  match = 0;
  for j=1:2
    for k=1:2
      check = sort([father(j), mother(k)]);
      if (check(1) == child(1)) && (check(2) == child(2))
	match = match + 1;
      endif
    endfor
  endfor
  %printf("match=%d\n", match); 
  genotypeFactor.val(i) = match / 4.0;
